...
class S2:
    ...
    For DetSector, local_conn returns the diagonal S_z^2 + (N_single / 2)
    term plus spin-exchange bras generated by S_- S_+ and S_+ S_-. For
    SpinSector, the sector is already spin-adapted, so S^2 is diagonal with
    eigenvalue S(S+1).
    """
    ...
    def diag(self, ket: Any) -> NDArray[np.float64]:
        ...
        if isinstance(self.sector, SpinSector):
            ...
            return np.full(ket.shape[0], s * (s + 1.0), dtype=np.float64)
    ...
    def local_conn(
        ...
        if isinstance(self.sector, SpinSector):
            return (
                diag,
                np.zeros(n_ket + 1, dtype=np.int64),
                ket[:0].copy(),
                np.empty(0, dtype=np.float64),
            )

...
def annihilate(x: Any, spin: int, p: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    mask = (np.uint64(1) << np.uint64(p & 63)) - np.uint64(1) if p & 63 else np.uint64(0)
    if mask:
...
class S2:
    ...
    def diag(self, x: Any) -> np.ndarray:
        if isinstance(self.space, CsfSpace):
            s = 0.5 * float(self.space.spin)
            return np.full(x.shape[0], s * (s + 1.0), dtype=np.float64)

        if isinstance(self.space, DetSpace):
            s_z = sz(x)
            n_alpha = _count(x[:, 0, :])
            n_beta = _count(x[:, 1, :])
        ...
        raise TypeError(f"unsupported space: {type(self.space).__name__}")

...
[[nodiscard]] inline int popcount_between_word(u64 occ, int i, int a) noexcept {
    ...
    const u64 hi_mask = hi >= 64 ? ~u64{0} : ((u64{1} << static_cast<unsigned>(hi)) - 1u);
...
[[nodiscard]] inline int popcount_between(std::span<const u64> occ, int i, int a) noexcept {
    ...
    if (w0 == w1) {
        ...
        return static_cast<int>(std::popcount(occ[static_cast<std::size_t>(w0)] & lo_mask & hi_mask));
    }

    int out = 0;
    out += static_cast<int>(std::popcount(occ[static_cast<std::size_t>(w0)] & (~u64{0} << b0)));
    for (int w = w0 + 1; w < w1; ++w) {
        out += static_cast<int>(std::popcount(occ[static_cast<std::size_t>(w)]));
    }
    const u64 hi_mask = b1 == 63 ? ~u64{0} : ((u64{1} << (b1 + 1u)) - 1u);
    out += static_cast<int>(std::popcount(occ[static_cast<std::size_t>(w1)] & hi_mask));
    return out;
}
...
inline void each_clear(std::span<const u64> occ, int norb, F&& f) {
    if (occ.size() == 1) {
        const u64 valid = norb >= 64 ? ~u64{0} : ((u64{1} << static_cast<unsigned>(norb)) - 1u);
    ...
    for (std::size_t w = 0; w < occ.size(); ++w) {
        ...
        if (rem <= 0) break;
        const u64 valid = rem >= 64 ? ~u64{0} : ((u64{1} << static_cast<unsigned>(rem)) - 1u);

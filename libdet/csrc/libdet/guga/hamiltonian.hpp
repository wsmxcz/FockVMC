...
using ConnSamples = rhf::ConnSamples;
using ProjectSamples = rhf::ProjectSamples;
...
inline Hamiltonian Hamiltonian::make(
    ...
    if (n_alpha < n_beta) {
        throw std::invalid_argument("Hamiltonian.guga: require n_alpha >= n_beta");
    }
    if (n_alpha < 0 || n_beta < 0 || n_alpha > norb || n_beta > norb) {
        throw std::invalid_argument("Hamiltonian.guga: invalid electron counts");
    }
